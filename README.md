...
```
- `read_blob`: reads file from specified ABFS path in Azure Storage and return its contents as bytes without mounting the container to a local filesystem
```
read_blob("file_path")
```
...
```
- `write_blob`: write bytes to a file in specified ABFS path
```
write_blob("data", "file_path")
```
- `write_blob_data`: write Pandas dataframe to a file in specified ABFS path
```
