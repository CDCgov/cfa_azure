...
class TestBatch(unittest.TestCase):
    ...
    @patch(
        "cfa_azure.helpers.get_batch_mgmt_client", MagicMock(return_value=True)
    )
    @patch(
        "cfa_azure.helpers.get_batch_pool_json", MagicMock(return_value=True)
    )
    @patch(
    ...
    def test_create_pool(self, mock_print):
        ...
        mock_print.assert_called_with(
            Contains("Pool creation process completed")
        )
    ...
    def test_run_job(self, mock_print):
        ...
        mock_print.assert_called_with(
            "Job complete. Time to debug. Job not deleted."
        )
    ...
    def test_package_and_upload_dockerfile(self, mock_print):
        ...
        mock_print.assert_called_with(
            "Dockerfile packaged and uploaded successfully."
        )
    ...
    def test_package_and_upload_dockerfile_failure(self, mock_print):
        cfa_azure.batch.package_and_upload_dockerfile(FAKE_CONFIG)
        mock_print.assert_called_with(
            "Failed to package and upload Dockerfile."
        )
